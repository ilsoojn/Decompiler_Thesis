...
condStr f g (v:vs) t
  ...
  | trace("c: " ++ (blockName basicB))(isCondBranch info) = do
    let condition = cond info
    ...
    if (isNothing tmp_v)
      then (txt basicB) ++ (condStr f g vs t)
        ...
        let tmpCond = state (fromJust tmp_v)
            ...
            (vsT, vsF) = bool (break (== (fromJust nfalse)) vs) (vs, []) (isNothing nfalse)
            (vsTrue, vsFalse) = bool (vsT, vsF) (vsF, vsT) (isConditionFalse)

            thenCode = concat $ subBlockToStr f g [vsTrue] t []
              --bool (otherStr vsT t) (otherStr vsF t) (isConditionFalse)
            elseCode = concat $  subBlockToStr f g [vsFalse] t []
              --bool (otherStr vsF t) (otherStr vsT t) (isConditionFalse)

        if trace((show v) ++ " T(" ++ (unwords $ map show vsTrue) ++ ") F(" ++ (unwords $ map show vsFalse) ++")")((not.null) elseCode)
...
otherStr (v:vs) t = do
  ...
  if (isNothing b)
    then (otherStr vs t)
    else trace("o: " ++ (blockName $fromJust b)) (txt $ fromJust b) ++ (otherStr vs t)
{-
